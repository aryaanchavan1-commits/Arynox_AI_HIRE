    ...
    <div className="relative flex min-h-screen items-center justify-center overflow-hidden bg-[#07070d] px-4 noise">
      ...
      <div className="relative z-10 w-full max-w-md">
        <div className="mb-8 text-center">
          ...
          <p className="mt-2 text-sm text-surface-400">Sign in to continue hiring with AI</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 rounded-3xl border border-white/10 bg-white/[0.04] p-8 shadow-2xl backdrop-blur-xl">
          <div>
            ...
            <div className="relative">
              ...
              <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@company.com"
                className="w-full rounded-xl border border-white/10 bg-white/5 py-2.5 pl-10 pr-4 text-sm text-white placeholder-surface-500 focus:border-indigo-500/60 focus:bg-white/[0.07] focus:outline-none transition-colors" required />
            </div>
          </div>
          <div>
            ...
            <div className="relative">
              ...
              <input type={showPassword ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Enter password"
                className="w-full rounded-xl border border-white/10 bg-white/5 py-2.5 pl-10 pr-10 text-sm text-white placeholder-surface-500 focus:border-indigo-500/60 focus:bg-white/[0.07] focus:outline-none transition-colors" required />
          ...
          <div className="relative my-5">
            ...
            <div className="relative flex justify-center text-xs uppercase tracking-wider">
              <span className="bg-transparent px-3 text-surface-600 backdrop-blur-sm">or continue with</span>
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <button type="button" className="rounded-xl border border-white/10 bg-white/5 py-2.5 text-sm font-medium text-white transition-colors hover:bg-white/10">Google</button>
            <button type="button" className="rounded-xl border border-white/10 bg-white/5 py-2.5 text-sm font-medium text-white transition-colors hover:bg-white/10">GitHub</button>
          </div>
        </form>

        <p className="mt-6 text-center text-sm text-surface-400">
          Don&apos;t have an account?{" "}
          <Link href="/auth/signup" className="font-medium text-indigo-400 hover:text-indigo-300">Sign up</Link>
        </p>
      </div>
    </div>
  );
